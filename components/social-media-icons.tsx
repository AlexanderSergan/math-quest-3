"use client"

import Link from "next/link"
import { Twitter, Youtube } from "lucide-react"
import { useIsChildRoute } from "@/hooks/use-is-child-route"

interface SocialMediaIconsProps {
  className?: string
  iconSize?: number
  showLabels?: boolean
  forceShow?: boolean
}

export function SocialMediaIcons({
  className = "",
  iconSize = 20,
  showLabels = false,
  forceShow = false,
}: SocialMediaIconsProps) {
  const isChildRoute = useIsChildRoute()

  // Don't render if on a child route, unless forceShow is true
  if (isChildRoute && !forceShow) {
    return null
  }

  return (
    <div className={`flex items-center space-x-4 ${className}`}>
      <Link
        href="https://x.com/mathquest"
        target="_blank"
        rel="noopener noreferrer"
        className="text-gray-700 hover:text-blue-600 transition-colors flex items-center"
        aria-label="Follow us on X"
      >
        <Twitter size={iconSize} />
        {showLabels && <span className="ml-2">X</span>}
      </Link>

      <Link
        href="https://tiktok.com/@mathquest"
        target="_blank"
        rel="noopener noreferrer"
        className="text-gray-700 hover:text-purple-600 transition-colors flex items-center"
        aria-label="Follow us on TikTok"
      >
        <TikTokIcon size={iconSize} />
        {showLabels && <span className="ml-2">TikTok</span>}
      </Link>

      <Link
        href="https://youtube.com/c/mathquest"
        target="_blank"
        rel="noopener noreferrer"
        className="text-gray-700 hover:text-red-600 transition-colors flex items-center"
        aria-label="Subscribe to our YouTube channel"
      >
        <Youtube size={iconSize} />
        {showLabels && <span className="ml-2">YouTube</span>}
      </Link>
    </div>
  )
}

// Custom TikTok icon since it's not in Lucide
function TikTokIcon({ size = 20 }) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="fill-current"
    >
      <path d="M19.589 6.686a4.793 4.793 0 0 1-3.77-4.245V2h-3.445v13.672a2.896 2.896 0 0 1-5.201 1.743l-.002-.001.002.001a2.895 2.895 0 0 1 3.183-4.51v-3.5a6.329 6.329 0 0 0-5.394 10.692 6.33 6.33 0 0 0 10.857-4.424V8.687a8.182 8.182 0 0 0 4.773 1.526V6.79a4.831 4.831 0 0 1-1.003-.104z" />
    </svg>
  )
}

